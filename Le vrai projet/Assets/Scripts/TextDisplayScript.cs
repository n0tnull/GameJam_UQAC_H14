...
public class TextDisplayScript : MonoBehaviour {
	...
	void OnGUI(){
		...
		{
			GUI.Label(new Rect(Screen.width/2-120,Screen.height/2-75,500,100), 
			          "Équipe " + race.GetTeam() + ", Appuyer sur Espace pour demarrer");
		}

		if (race.TimeBeforeStart > 0 && race.TimerVisible){
			GUI.Label(new Rect(Screen.width/2-5,Screen.height/2-75,500,100), "" + (int) race.TimeBeforeStart);
		}

		if(playerPhysics.dead && race.GetTeam() == 1){
			GUI.Label(new Rect(Screen.width/2-75,Screen.height/2-heightOffset,500,100), 
			          "Vous etes morts!\nRespawn dans " + (int)playerDeath.timeSinceDeath + " secondes.");
		}

		if(playerPhysics.dead && race.GetTeam() == 2){
			GUI.Label(new Rect(Screen.width/2-50,Screen.height/2-heightOffset,500,100), 
